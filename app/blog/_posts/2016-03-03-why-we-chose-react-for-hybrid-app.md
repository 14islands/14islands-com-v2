...
**We recently had the pleasure of working with [Fjord](https://www.fjordnet.com/) to prototype and build an HTML5 hybrid app for one of their clients.**
...
In this article we share the techniques and challenges we faced while building an app for iOS and Android using [React](https://facebook.github.io/react/) &amp; [Cordova](https://cordova.apache.org/).
...
We’ve ventured down this path [many times](http://blog.14islands.com/post/52546836134/case-study-betting-on-a-fully-responsive-web) in [the past](http://blog.14islands.com/post/52934733389/racer-a-chrome-experiment) using more conventional JavaScript MVC frameworks such as Backbone.js. 
...
## Enter React
[React](https://facebook.github.io/react/) is a JavaScript library for creating user interfaces and is often described as the *V* in *MVC*. The idea behind React is to build reusable components that automatically manage all UI updates.
...
We are big fans of modern JavaScript and opted for writing ES2015 syntax using [Babel](https://babeljs.io/).

Mixins cannot be used with ES2015 classes and there are other [good reasons not to use them](https://medium.com/@dan_abramov/mixins-are-dead-long-live-higher-order-components-94a0d2f9e750#.6uq9fmvgn). We used Higher-order-components to compose functionality instead of mixins:
...
There are endless implementations of Flux and after researching many of them we settled on using [*Alt* ](https://github.com/goatslacker/alt).
...
This can quickly get a bit messy and verbose for a component with many state modifiers, so we created our own [bem-helper](https://github.com/14islands/bem-helper-js) to make it easier to work with BEM class names in JSX.
...
Luckily the React team has already provided us with the [ReactCSSTransitionGroup](https://facebook.github.io/react/docs/animation.html) which solves the most common scenario of applying animation classes and adding/removing animating elements from the DOM. It worked great for handling the page transitions in our app.


## Wrapping it up
We used [Apache Cordova](https://cordova.apache.org/) to package the app for iOS and Android. It's fairly straightforward to setup, and it comes with a bunch of useful plugins that expose a JavaScript API to access certain native features. 

As an example, we included the [Statusbar plugin](https://github.com/apache/cordova-plugin-statusbar) to change the color of the native statusbar during runtime.
...
There is an official [cordova plugin for the new WKWebView](https://github.com/apache/cordova-plugin-wkwebview-engine) for iOS 9 users. However, it's not possible to use XHR from the file:// protocol without CORS enabled which prevented us from using it.
